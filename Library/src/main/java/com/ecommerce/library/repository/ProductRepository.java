...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    /* CUSTOMER */
    @Query(value = "select p from Product p where p.is_activated = true and p.is_deleted = false")
    List<Product> getAllProducts();

    @Query(value = "select * from products p where p.is_deleted = false and p.is_activated = true order by rand() asc limit 4 ",
            nativeQuery = true)
